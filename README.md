...
**DNDGame** — это десктопное приложение с GUI, БД и событийно-ориентированной архитектурой, построенное на базе `customtkinter`. Проект находится в активной разработке и позволяет создавать персонажей, распределять их характеристики, совершать броски кубиков и проходить приключения с ветвящимся сюжетом.
...
- [Настройка базы данных](#%EF%B8%8F-настройка-базы-данных)
- [Что уже сделано](#-что-уже-сделано)
- [Планы по развитию](#%EF%B8%8F-планы-по-развитию)
  - [Ближайшие задачи](#ближайшие-задачи-приоритет-высокий)
  - [Будущие версии](#будущие-версии)
- [Техническая архитектура (планируемая)](#техническая-архетиктура-планируемая)
...
Таблица journey (история приключений)
Хранит в себе историю приключений пользователей

| Поле       | Тип     | описание                                       | Ограничения           |
|------------|---------|------------------------------------------------|-----------------------|
| id         | SERIAL  | Уникальный идентификатор                       | PRIMARY KEY           |
| id_event   | INTEGER | Название события                               | REFERENCES events(id) |
| id_user    | INTEGER | Описание события                               | REFERENCES users(id)  |
| current_hp | INTEGER | URL изображения для события                    | -                     |
| enemy_hp   | INTEGER | Сложность проверки силы                        | -                     |
| status     | INTEGER | Сложность проверки харизмы                     | -                     |
...
| 6 | Окно сражения                          | Визуальная часть сражения                                                  |
| 7 | Покрыл часть кода Unit тестами         | Покрыл код unit тестами на базе фреймворка pytest                          |

## 🗺️ Планы по развитию

## Ближайшие задачи (Приоритет: высокий)

□ Экран поражения — модальное окно при гибели персонажа

□ Экран победы — информативный экран при успешном завершении битвы
